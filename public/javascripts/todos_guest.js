...
var TaskController = function() {
  ...
  Constructor.prototype.load = function() {
    var self = this;
    $.getJSON("/tasks_guest", function(data) {
      self.tasks_guest = data;
      self.render();
      self.clearForm();
    });
  };
  ...
  Constructor.prototype.render = function() {
    var self = this;
    $("#main").toggleClass("no-task", (this.tasks_guest.length <= 0));
    var html = _.map(this.tasks_guest, function(task) {
  ...
  Constructor.prototype._findTask = function(e) {
    ...
    var id = el.data('id');
    return  _.find(this.tasks_guest, {id: id});
  };
  ...
  Constructor.prototype.postTask = function() {
    var self = this;
    $.post("/tasks_guest", $("#form-task").serialize(), function(data) {
      console.log(data);
      self.tasks_guest.push(data);
      self.render();
      self.clearForm();
    });
  };
  ...
  Constructor.prototype.removeTask = function(e) {
    ...
    if (confirm('정말로 삭제하시겠습니까?')) {
      $.ajax({
        ...
        dataType: 'json',
        success: function(data) {
          self.tasks_guest = _.reject(self.tasks_guest, function(t) {
            return t.id === task.id;
          });
          var el = $(e.currentTarget).closest('li');
          el.remove();
